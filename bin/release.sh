...
export LATEST_TAG=$(curl -H \
  ...
  https://api.github.com/repos/markdboyd/cumulus-message-adapter-js/tags | jq --raw-output '.[0].name')
...
export RELEASE_URL=$(curl -H \
  "Authorization: token $GITHUB_TOKEN" \
  https://api.github.com/repos/markdboyd/cumulus-message-adapter-js/releases/tags/$VERSION_TAG | jq --raw-output '.url // ""')
